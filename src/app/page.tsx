...
/* ═══════════════════════════════════════════════════════════
   ...
   Animation engine: Framer Motion spring physics (matching Reference.me)
   Supplementary: GSAP DrawSVG + ScrambleText for frame content

   Architecture:
   1. Ghost div creates native scrollable area (both X and Y)
   2. useScroll tracks scrollX / scrollY
   3. Scale is a useSpring motion value
   4. Horizontal position is a motion value driven by scroll
   5. Hero intro uses spring animations (NOT tweens)
   ═══════════════════════════════════════════════════════════ */
...
export default function Home() {
  ...
  // ── Compute layout on mount + resize ──
  useEffect(() => {
    function calc() {
      ...
      const maxScale = clamp(
        Math.min(vw / 1300, vh / 1020),
        [0.2, 1],
      );

      // On first render, jump spring to maxScale
  ...
  function handleScroll(scrollVal: number) {
    ...
    // Reference's exact logic: default to maxScale, only decrease when scrolling
    let targetScale = maxScale;
    if (scrollVal > 0) {
      targetScale = clamp(
        scaleSpring.get() + -0.01 * scrollVal * 0.01,
        [0.7, maxScale],
      );
    }
    // Always set — when scrollVal===0, spring animates back to maxScale (hero fills screen)
  ...
  // ── Listen to native scroll events (both X and Y drive horizontal movement) ──
  useEffect(() => {
    const update = () => {
  ...
  // ── Frame label counter-scale (stay readable at any zoom) ──
  const labelScale = useTransform(scaleSpring, s => 1 / s);
  const labelTop = useTransform(scaleSpring, s => -28 / s);

  // ── Frame-level content reveals (GSAP for DrawSVG/ScrambleText) ──
  ...
  const frameLeft = (i: number) => FRAME_STEP * i;
  ...
  return (
    ...
    >
      {/* Ghost div — creates native scrollable area */}
      <div
        aria-hidden
        className="ghost"
        style={{
        ...
        >
          ...
          <Frame index={1} labelScale={labelScale} labelTop={labelTop} label="The Spark">
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: "100%", height: "100%", gap: 28, padding: "0 80px" }}>
              <svg width="440" height="320" viewBox="0 0 440 320" fill="none" style={{ opacity: 0.9 }}>
                ...
                {/* === Art on screen (orange) — spirograph / algorithmic curves === */}
                {/* Curve 1: large flowing wave */}
          ...
          <Frame index={2} labelScale={labelScale} labelTop={labelTop} label="The Hacker" bg="#0a0a0a" labelColor="rgba(255,255,255,0.35)">
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", width: "100%", height: "100%", padding: "0 72px", gap: 6, color: "white" }}>
              <div data-reveal style={{ marginLeft: "4%" }}>
                <span style={{ display: "block", fontSize: 96, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1 }}>HACKED</span>
              </div>
              <div data-reveal style={{ marginLeft: "16%" }}>
                <span style={{ display: "block", fontSize: 96, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1, color: "var(--color-orange)" }}>THE SYSTEM</span>
              </div>
              <div data-reveal style={{ marginLeft: "8%", marginTop: 16 }}>
                <span style={{ display: "block", fontSize: 96, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1, color: "rgba(255,255,255,0.15)" }} data-scramble="THEN BUILT">████████████████</span>
              </div>
          ...
          <Frame index={3} labelScale={labelScale} labelTop={labelTop} label="The Builder">
            <div style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "center", width: "100%", height: "100%", padding: "0 72px" }}>
              ...
              <div data-reveal style={{ marginLeft: "4%" }}>
                <span style={{ display: "block", fontSize: 120, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1 }}>BUILT</span>
              </div>
              <div data-reveal style={{ marginLeft: "16%" }}>
                <span style={{ display: "block", fontSize: 120, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1, color: "var(--color-orange)" }}>MACHINES</span>
              </div>
          ...
          <Frame index={6} labelScale={labelScale} labelTop={labelTop} label="2026" bg="#0a0a0a" labelColor="rgba(255,255,255,0.35)">
            <div style={{ position: "relative", display: "flex", flexDirection: "column", justifyContent: "center", width: "100%", height: "100%", padding: "0 72px" }}>
              ...
              <div data-reveal style={{ marginLeft: "4%" }}>
                <span style={{ display: "block", fontSize: 120, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1, color: "white" }}>AI</span>
              </div>
              <div data-reveal style={{ marginLeft: "16%" }}>
                <span style={{ display: "block", fontSize: 120, fontWeight: 700, letterSpacing: "-0.05em", lineHeight: 1, color: "var(--color-orange)" }}>SOFTWARE</span>
              </div>
...
/* ─────────────────────────────────────────────────────────
   ClipReveal — Reference's exact text reveal (spring physics)
   ───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   Frame — Reusable frame wrapper with counter-scaled label
   ───────────────────────────────────────────────────────── */
function Frame({
  children,
  index,
  ...
  return (
    <div
      className="frame"
      style={{ left: index * FRAME_STEP, background: bg || "var(--color-sheet)" }}
    >
      {label && (
        <motion.div
          aria-hidden
          className="frame-label"
          style={{ scale: labelScale, top: labelTop, color: labelColor }}
        >
          {label}
        </motion.div>
      )}
...
/* ─────────────────────────────────────────────────────────
   Minimap — Lines + tracker + section label
   ───────────────────────────────────────────────────────── */
...
function Minimap({
  ...
  const trackerXTarget = useTransform(posX, (x) => {
    ...
    const currentLine = Math.round(progress * (LINE_COUNT - 1));
    return currentLine * (1 + LINE_GAP);
  });

  const trackerX = useSpring(trackerXTarget, { stiffness: 400, damping: 40 });

  // Track current section for label
  ...
  useEffect(() => {
    const unsub = sectionIndex.on("change", (v) => {
      setSectionName(SECTION_NAMES[Math.round(v)] || "");
    });
    return unsub;
  }, [sectionIndex]);
...
/* ─────────────────────────────────────────────────────────
   CrossCursor — follows mouse with spring physics
   ───────────────────────────────────────────────────────── */
function CrossCursor() {
  ...
  return (
    <motion.svg
      className="cross"
      style={{ x: springX, y: springY, opacity: 0.6 }}
      width="20"
      height="20"
      viewBox="0 0 30 30"
      fill="none"
